...
mkValidator :: () -> Integer -> ScriptContext -> Bool --comes at cost uses more resources as compared to using BuiltinTypes and requires additional code
mkValidator _ r _ = traceIfFalse "wrong redeemer" $ r == 42 --Ignores string when True but logs string when False
...
give amount = do
    let tx = mustPayToTheScript () $ Ada.lovelaceValueOf amount -- mustPayToTheScript when only one script is used for the transaction.
