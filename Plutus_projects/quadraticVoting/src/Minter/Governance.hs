...
-- Validating if 
      ...
      hasUTxO =
        -- {{{
        utxoIsGettingSpent (txInfoInputs info) oref --Transaction inputs vs reference to utxos.
      ...
      checkMintedAmount =
        ...
        [(_, dlTN', dlAmt'), (_, tn', amt')] ->
          -- {{{
          traceIfFalse "Bad deadline token name." (dlTN' == dlTN)
            && traceIfFalse "Bad main token name." (tn' == tn)
            && traceIfFalse "Exactly 1 deadline token must be minted."
                            (dlAmt' == 1)
            && traceIfFalse "Exactly 1 main token must be minted" (amt' == 1)
        -- }}}
        _ ->
          -- {{{
...
qvfPolicy oref deadline =
  -- {{{
  let wrap :: (TempRedeemer -> ScriptContext -> Bool) -> PSU.V2.UntypedMintingPolicy
      wrap = PSU.V2.mkUntypedMintingPolicy
   in Plutonomy.optimizeUPLC $
        mkMintingPolicyScript $
          $$(PlutusTx.compile [||\oref' deadline' dlTN' tn' -> wrap $ mkQVFPolicy oref' deadline' dlTN' tn'||])
            ...
            `PlutusTx.applyCode` PlutusTx.liftCode deadlineTokenName
            `PlutusTx.applyCode` PlutusTx.liftCode qvfTokenName
