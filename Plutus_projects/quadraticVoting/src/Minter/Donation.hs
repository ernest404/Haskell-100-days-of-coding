...
mkDonationPolicy :: CurrencySymbol -> DonationRedeemer -> ScriptContext -> Bool
        ...
        let
          ...
          outputSAndVAreValid s v =
            ...
            traceIfFalse
                ...
                (validateGovUTxO (txOutValue inputProjUTxO)
                                 (txOutAddress inputProjUTxO)
                                 (ReceivedDonationsCount $ currDCount + 1)
                                 s
                )
              && traceIfFalse "Invalid value for the donation UTxO."
        ...
        let
          ...
          foldDonationsPhaseTwo requiredDonationCount =
            ...
            let
              ...
              updatedDatum = PrizeWeight (foldDonationsMap finalMap) False
        ...
        in
          case getInlineDatum inputProjUTxO of
            ReceivedDonationsCount tot ->
              -- {{{
              if tot <= maxDonationInputsForPhaseTwo
                then foldDonationsPhaseTwo tot
                else traceError
                  "Donation count is too large for direct burning."
              -- }}}
            DonationFoldingProgress tot soFar ->
              ...
              then foldDonationsPhaseTwo tot
                ...
                "All donation tokens must be folded before burning."
              -- }}}
            _ ->
              -- {{{
              traceError
